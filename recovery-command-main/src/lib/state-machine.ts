...
const T: Record<CaseStatus, Transition[]> = {
  ...
  ASSIGNED: [
    { to: "SOFT_COLLECTION", roles: ["COLLECTOR", "BANK_ADMIN"], label: "Начать софт-взыскание" },
    { to: "ESCALATED_TO_LEGAL", roles: ["BANK_ADMIN"], label: "Эскалация в юр. отдел" },
  ],
  SOFT_COLLECTION: [
    { to: "CONTACTED", roles: ["COLLECTOR"], label: "Контакт установлен" },
    { to: "NO_CONTACT", roles: ["COLLECTOR"], label: "Нет контакта" },
    { to: "PROMISE_TO_PAY", roles: ["COLLECTOR"], label: "Обещание оплаты" },
    { to: "DISPUTE", roles: ["COLLECTOR", "BANK_ADMIN"], label: "Спор" },
    { to: "ESCALATED_TO_LEGAL", roles: ["BANK_ADMIN"], label: "Эскалация" },
  ],
  CONTACTED: [
    { to: "PROMISE_TO_PAY", roles: ["COLLECTOR"], label: "Обещание оплаты" },
    { to: "PARTIALLY_PAID", roles: ["COLLECTOR"], label: "Частичная оплата" },
    { to: "DISPUTE", roles: ["COLLECTOR"], label: "Спор" },
  ],
  NO_CONTACT: [
    { to: "SOFT_COLLECTION", roles: ["COLLECTOR"], label: "Возобновить работу" },
    { to: "ESCALATED_TO_LEGAL", roles: ["BANK_ADMIN"], label: "Эскалация" },
  ],
  PROMISE_TO_PAY: [
    { to: "PARTIALLY_PAID", roles: ["COLLECTOR"], label: "Частичная оплата" },
    { to: "PAID", roles: ["COLLECTOR"], label: "Полная оплата" },
    { to: "PROMISE_BROKEN", roles: ["COLLECTOR"], label: "Обещание нарушено" },
  ],
  PROMISE_BROKEN: [
    { to: "SOFT_COLLECTION", roles: ["COLLECTOR"], label: "Продолжить" },
    { to: "ESCALATED_TO_LEGAL", roles: ["BANK_ADMIN"], label: "Эскалация" },
  ],
  PARTIALLY_PAID: [
    { to: "PAID", roles: ["COLLECTOR"], label: "Полная оплата" },
